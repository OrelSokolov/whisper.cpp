...
RSpec.describe 'Punctuation in transcription via worker', type: :integration do
  TIMEOUT_SECONDS = 20
  RUNS_COUNT = 10
  ...
  before(:all) do
    ...
    whisper_worker_path = File.join(project_root, 'build', 'bin', 'whisper-worker')
    client_rb_path = File.join(project_root, 'examples', 'worker', 'client.rb')
    audio_file_path = File.expand_path('~/tube/babes.mp3')
    model_path = File.join(project_root, 'models', 'ggml-large-v3-turbo.bin')

    unless File.exist?(whisper_worker_path)
      skip 'whisper-worker not built. Run: rake build:worker'
    end

    unless File.exist?(client_rb_path)
      skip 'client.rb not found at examples/worker/client.rb'
    end
  ...
  it "should produce punctuation in at least #{MIN_SUCCESSFUL_RUNS} out of #{RUNS_COUNT} runs" do
    ...
    begin
      runs_count.times do |run_number|
        puts "\n[Run #{run_number + 1}/#{runs_count}] Starting worker and client..."
        ...
        begin
          ...
          puts "  ✗ Failed to start worker: #{e.message}"
          failed_runs << { run: run_number + 1, error: "Worker start failed: #{e.message}" }
          worker_info = nil
          next
        end
        ...
        unless worker_info[:wait_thr].alive?
          puts "  ✗ Worker died immediately after start"
          failed_runs << { run: run_number + 1, error: "Worker died immediately after start" }
        ...
        # Останавливаем worker перед следующим запуском
        stop_worker(worker_info)
        worker_info = nil

        # Ждем, пока порт освободится перед следующим запуском
        max_wait = 3
        waited = 0
        while waited < max_wait
          begin
      ...
      # Убеждаемся, что worker остановлен
      stop_worker(worker_info) if worker_info
    end
